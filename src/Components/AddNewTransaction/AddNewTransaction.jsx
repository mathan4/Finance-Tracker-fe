...
const AddNewTransaction = () => {
    ...
    const transactionAmountHandler=(event)=>{
        setTransactionData({...transactionData,transactionAmount:event.target.value});
    }

    const transactionDescriptionHandler=(event)=>{
        setTransactionData({...transactionData,transactionDescription:event.target.value});
    }
    const transactionDateHandler=(event)=>{
        setTransactionData({...transactionData,transactionDate:event.target.value});
    }

   const formSubmitHandler=(event)=>{
    event.preventDefault();
    try {
        axios.post('http://localhost:3500/api/v1/financetracker/add', transactionData)
          .then(response => {
